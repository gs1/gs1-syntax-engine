 ...
 *
 */
GS1_SYNTAX_DICTIONARY_API gs1_lint_err_t gs1_lint_gcppos2(const char* const data, size_t data_len, size_t* const err_pos, size_t* const err_len)
{
	...
	ret = gs1_lint_gcppos1(data + 1, data_len - 1, err_pos, err_len);
...
void test_lint_gcppos2(void)
{
	 ...
	 *  I0     |  => Short; I*0*
	 */
	for (i = GCP_MIN_LENGTH; i >= 2; i--) {
		data[i] = '\0';
		expect[0] = 'I';
		expect[1] = '*';
		memcpy(expect+2, data+1, (size_t)i-1);
		expect[i+1] = '*';
		expect[i+2] = '\0';
